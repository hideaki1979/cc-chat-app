...
// QueryRoomMembers chains the current query on the "room_members" edge.
func (crq *ChatRoomQuery) QueryRoomMembers() *RoomMemberQuery {
	query := (&RoomMemberClient{config: crq.config}).Query()
...
func (crq *ChatRoomQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (crq *ChatRoomQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (crq *ChatRoomQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (crq *ChatRoomQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (crq *ChatRoomQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (crq *ChatRoomQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := crq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (crq *ChatRoomQuery) Clone() *ChatRoomQuery {
	...
	return &ChatRoomQuery{
		...
		withRoomMembers: crq.withRoomMembers.Clone(),
...
// WithRoomMembers tells the query-builder to eager-load the nodes that are connected to
// the "room_members" edge. The optional arguments are used to configure the query builder of the edge.
func (crq *ChatRoomQuery) WithRoomMembers(opts ...func(*RoomMemberQuery)) *ChatRoomQuery {
	query := (&RoomMemberClient{config: crq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	crq.withRoomMembers = query
	return crq
}
...
func (crq *ChatRoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ChatRoom, error) {
	...
	if query := crq.withRoomMembers; query != nil {
		if err := crq.loadRoomMembers(ctx, query, nodes,
			func(n *ChatRoom) { n.Edges.RoomMembers = []*RoomMember{} },
			func(n *ChatRoom, e *RoomMember) { n.Edges.RoomMembers = append(n.Edges.RoomMembers, e) }); err != nil {
			return nil, err
		}
	}
...
func (crq *ChatRoomQuery) loadRoomMembers(ctx context.Context, query *RoomMemberQuery, nodes []*ChatRoom, init func(*ChatRoom), assign func(*ChatRoom, *RoomMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*ChatRoom)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(roommember.FieldRoomID)
	}
	query.Where(predicate.RoomMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chatroom.RoomMembersColumn), fks...))
	}))
...
func (crq *ChatRoomQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(chatroom.Table, chatroom.Columns, sqlgraph.NewFieldSpec(chatroom.FieldID, field.TypeUUID))
	_spec.From = crq.sql
