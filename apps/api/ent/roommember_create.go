...
func (rmc *RoomMemberCreate) SetRoomID(u uuid.UUID) *RoomMemberCreate {
	rmc.mutation.SetRoomID(u)
	return rmc
}

// SetUserID sets the "user_id" field.
func (rmc *RoomMemberCreate) SetUserID(u uuid.UUID) *RoomMemberCreate {
	rmc.mutation.SetUserID(u)
	return rmc
}
...
func (rmc *RoomMemberCreate) SetID(i int64) *RoomMemberCreate {
	rmc.mutation.SetID(i)
	return rmc
}

// SetRoom sets the "room" edge to the ChatRoom entity.
func (rmc *RoomMemberCreate) SetRoom(c *ChatRoom) *RoomMemberCreate {
	return rmc.SetRoomID(c.ID)
}
...
func (rmc *RoomMemberCreate) check() error {
	...
	if v, ok := rmc.mutation.ID(); ok {
		if err := roommember.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`ent: validator failed for field "RoomMember.id": %w`, err)}
		}
	}
...
func (rmc *RoomMemberCreate) createSpec() (*RoomMember, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(roommember.Table, sqlgraph.NewFieldSpec(roommember.FieldID, field.TypeInt64))
	)
